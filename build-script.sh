...
INTERFACEVIEW=gr740-round_trip_time_iv.aadl
grep "version => \"2" gr740-round_trip_time_iv.aadl >/dev/null && {
    echo -e "${INFO} Converting interface view from V2 to V1.3"
    TASTE --load-interface-view gr740-round_trip_time_iv.aadl --export-interface-view-to-1_3 __iv_1_3.aadl
...
cd "$SKELS" && rm -f sender.zip && zip sender sender/* && cd $OLDPWD

cd "$SKELS" && rm -f receiver.zip && zip receiver receiver/* && cd $OLDPWD
...
cd "$CWD" && assert-builder-ocarina.py \
	...
	-o "$OUTPUTDIR" \
	--subC sender:"$SKELS"/sender.zip \
	--subC receiver:"$SKELS"/receiver.zip \
